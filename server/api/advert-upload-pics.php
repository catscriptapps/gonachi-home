...
//
// Uploads one or more pictures to an advert (owner-only, capped at
// getMediaLimit()). Called by the shared upload-modal.js from inside the
// view-advert modal's "Add Photo" button.
...
// id arrives via the query string (createUploadHandler's endpointUrl),
// not the multipart body.
...
$result = AdvertsPicturesController::store($id, $userId, $_FILES['images']);
