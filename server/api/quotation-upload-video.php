<?php
// /server/api/quotation-upload-video.php
//
// Chunked video upload for a quotation (owner-only). Mirrors
...
// on the client). At most one video per quotation — a successful upload
// here always replaces whichever video is already attached (see
// QuotationsController::attachVideo()).
...
try {
    $uploadDir = __DIR__ . '/../../public/videos/quotations/';
    // Max 100MB — matches Adverts' cap for the same reason (short project clips, not full-length videos).
    ...
    $attach = QuotationsController::attachVideo($encodedId, $userId, $result['fileName']);

    if (!$attach['success']) {
        // Roll back the just-uploaded file — the quotation lookup/ownership failed.
    ...
    json_response([
        ...
        'url' => getAssetBase() . 'videos/quotations/' . $result['fileName'],
        'files' => [['url' => getAssetBase() . 'videos/quotations/' . $result['fileName']]],
    ]);
} catch (\Throwable $e) {
