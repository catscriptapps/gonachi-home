<?php
// /resources/views/partials/real-estate-world-header.php

declare(strict_types=1);

/**
 * Gonachi Real Estate World - Core Dynamic Topbar
 */
?>
<header class="h-20 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 flex items-center justify-between px-4 sm:px-6 lg:px-8 sticky top-0 z-40 transition-colors duration-300">
    ...
    <div class="flex items-center space-x-4">
        ...
        <div class="hidden sm:flex items-center gap-2">
            <div class="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 bg-teal-50 dark:bg-teal-950/40 text-teal-600 dark:text-teal-400">
                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
            </div>
            <span class="text-sm font-semibold text-gray-500 dark:text-gray-400">
                Real Estate Stakeholder Network
            </span>
        </div>
    </div>
    ...
    <div class="flex items-center space-x-4">
        ...
        <?php if ($isLoggedIn): ?>
            <a href="#" data-logout-button class="flex items-center space-x-3 group cursor-pointer">
                <div id="header-account-avatar" data-initial="<?= htmlspecialchars($accountInfo['initial']) ?>" data-accent-class="bg-teal-600" class="w-8 h-8 rounded-full <?= empty($accountInfo['avatarUrl']) ? 'bg-teal-600' : '' ?> text-white font-bold flex items-center justify-center text-sm shadow-sm overflow-hidden">
                ...
                <div class="hidden md:flex flex-col leading-tight">
                    <span class="text-sm font-medium text-gray-700 dark:text-gray-300 group-hover:text-teal-600 dark:group-hover:text-teal-400 transition-colors"><?= htmlspecialchars($accountInfo['displayName']) ?></span>
                    <span class="text-xs text-gray-400 group-hover:text-teal-500 dark:group-hover:text-teal-400 transition-colors">Sign Out</span>
                </div>
            </a>
        <?php else: ?>
            <a href="<?= $baseUrl ?>login" data-login-button class="flex items-center space-x-3 group cursor-pointer">
                ...
                <span class="text-sm font-medium text-gray-700 dark:text-gray-300 group-hover:text-teal-600 dark:group-hover:text-teal-400 hidden md:block transition-colors">Sign In</span>
            </a>
        <?php endif; ?>
    </div>
</header>
