    ...
    <div class="flex items-center space-x-4">
        ...
        <?php if ($isLoggedIn): ?>
            <a href="#" data-logout-button class="flex items-center space-x-3 group cursor-pointer">
                <div id="header-account-avatar" data-initial="<?= htmlspecialchars($accountInfo['initial']) ?>" data-accent-class="bg-indigo-600" class="w-8 h-8 rounded-full <?= empty($accountInfo['avatarUrl']) ? 'bg-indigo-600' : '' ?> text-white font-bold flex items-center justify-center text-sm shadow-sm overflow-hidden">
