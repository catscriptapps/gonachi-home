<?php
// /resources/views/partials/swap-header.php
...
<header class="h-20 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 flex items-center justify-between px-4 sm:px-6 lg:px-8 sticky top-0 z-40 transition-colors duration-300">
    ...
    <div class="flex items-center space-x-4">
        ...
        <div class="hidden sm:flex items-center gap-2">
            <div class="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 bg-purple-50 dark:bg-purple-950/40 text-purple-600 dark:text-purple-400">
    ...
    <div class="flex items-center space-x-4">
        ...
        <?php if ($isLoggedIn): ?>
            <a href="#" data-logout-button class="flex items-center space-x-3 group cursor-pointer">
                <div id="header-account-avatar" data-initial="<?= htmlspecialchars($accountInfo['initial']) ?>" data-accent-class="bg-purple-600" class="w-8 h-8 rounded-full <?= empty($accountInfo['avatarUrl']) ? 'bg-purple-600' : '' ?> text-white font-bold flex items-center justify-center text-sm shadow-sm overflow-hidden">
                ...
                <div class="hidden md:flex flex-col leading-tight">
                    <span class="text-sm font-medium text-gray-700 dark:text-gray-300 group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors"><?= htmlspecialchars($accountInfo['displayName']) ?></span>
                    <span class="text-xs text-gray-400 group-hover:text-purple-500 dark:group-hover:text-purple-400 transition-colors">Sign Out</span>
                </div>
            </a>
        <?php else: ?>
            <a href="<?= $baseUrl ?>login" data-login-button class="flex items-center space-x-3 group cursor-pointer">
                ...
                <span class="text-sm font-medium text-gray-700 dark:text-gray-300 group-hover:text-purple-600 dark:group-hover:text-purple-400 hidden md:block transition-colors">Sign In</span>
            </a>
        <?php endif; ?>
    </div>
</header>
