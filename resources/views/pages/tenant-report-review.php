<?php
// /resources/views/pages/tenant-report-review.php

declare(strict_types=1);

/**
 * Admin-only moderation queue for submitted tenant reports (status =
 * pending_review). Approving here is what makes a report count toward a
 * tenant's public confidence score and search results. Exact mirror of
 * landlord-report-review.php for the opposite report direction.
 *
 * @var bool $isLoggedIn
 * @var string $baseUrl
 */
...
$pendingReports = TenantReportReviewController::pending(15);

$conductLabels = [
    'payment_default' => 'Late / Defaulted Rent Payment',
...
<div class="space-y-6">
    ...
    $breadcrumbs = [
        ...
        ['label' => 'Tenant Report Review Queue'],
    ];
    ...
    <div>
        <h1 class="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Tenant Report Review Queue</h1>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Approve submitted reports to count them toward a tenant's public confidence score, or reject noise before it goes live.
    ...
    <?php else: ?>
        <div class="space-y-4">
            <?php foreach ($pendingReports as $report): ?>
                <?php $conduct = $conductLabels[$report->conduct_type] ?? ucfirst($report->conduct_type); ?>
                <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-5 shadow-sm">
                    <div class="flex items-start justify-between gap-4 mb-3">
                        <div>
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800 dark:bg-indigo-950 dark:text-indigo-400">
                                <?= htmlspecialchars($conduct) ?>
                            </span>
                            <h4 class="text-base font-bold text-gray-900 dark:text-white mt-2">
                                <?= htmlspecialchars($report->tenant->name ?? 'Unknown Tenant') ?>
                            </h4>
                            <p class="text-sm text-gray-500 dark:text-gray-400"><?= htmlspecialchars($report->property_address ?? 'No address on file') ?></p>
                        </div>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 my-4 text-sm border-t border-b border-gray-100 dark:border-gray-800/80 py-3">
                        ...
                        <div>
                            <span class="block text-xs font-semibold text-gray-400 uppercase tracking-wider">Reference</span>
                            <span class="font-medium text-gray-700 dark:text-gray-300"><?= htmlspecialchars($report->reference_name ?? '—') ?></span>
                        </div>
                    </div>
